    ...
    <AdminShell title="Reports">
      <div className="space-y-8 max-w-6xl">
        ...
        <section className="grid grid-cols-2 lg:grid-cols-4 gap-4">
          {SUMMARY_CARDS.map((s) => (
            <div key={s.label} className="bg-white rounded-2xl border border-[#e8e8e8] p-6">
              <p className="text-[11px] font-semibold text-[#444748] uppercase tracking-wider">{s.label}</p>
              <h3 className="font-['Playfair_Display'] text-[28px] font-semibold text-black mt-1">{s.value}</h3>
              <p className={`text-[12px] font-semibold flex items-center gap-1 mt-2 ${s.noSign ? "text-[#444748]" : s.delta >= 0 ? "text-[#41379b]" : "text-[#ba1a1a]"}`}>
                {!s.noSign && (
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          {/* Sales Chart */}
          <section className="lg:col-span-2 bg-white rounded-2xl border border-[#e8e8e8] p-6">
            <div className="flex items-center justify-between mb-6">
              <div>
                <h3 className="font-semibold text-black text-[18px]">Sales Growth</h3>
                <p className="text-[#444748] text-[13px]">Current vs. previous year — same months</p>
              </div>
              <div className="flex items-center gap-4 text-[11px]">
                <span className="flex items-center gap-1.5"><span className="w-5 h-0.5 bg-black inline-block" /> This year</span>
                <span className="flex items-center gap-1.5"><span className="w-5 h-0.5 bg-[#c4c7c7] inline-block border-dashed" style={{ borderTopStyle: "dashed", borderTopWidth: 2 }} /> Last year</span>
              </div>
            </div>

            {chartData.every((d) => d.curr === 0 && d.prev === 0) ? (
              <div className="h-52 flex items-center justify-center text-[#747878] text-sm">No sales data yet.</div>
            ) : (
              <div className="relative h-52">
                <svg viewBox="0 0 540 200" className="w-full h-full overflow-visible">
                  {[0, 50, 100, 150, 200].map((y) => (
                    <line key={y} x1="0" y1={y} x2="540" y2={y} stroke="#eeeeee" strokeWidth="1" />
                  ))}
                  <polyline
                    points={chartData.map((d, i) => `${i * (540 / Math.max(chartData.length - 1, 1))},${200 - (d.prev / maxVal) * 190}`).join(" ")}
                    fill="none" stroke="#c4c7c7" strokeWidth="2" strokeDasharray="6,4"
                  />
                  <polyline
                  ...
                  />
                  {chartData.map((d, i) => (
                    <circle key={i} cx={i * (540 / Math.max(chartData.length - 1, 1))} cy={200 - (d.curr / maxVal) * 190} r="4" fill="black">
                ...
                <div className="absolute bottom-0 left-0 right-0 flex justify-between px-1">
                  {chartData.map((d) => (
                    <span key={d.month} className="text-[11px] text-[#444748]">{d.month}</span>
                  ))}
                </div>
              </div>
            )}
          ...
          <section className="space-y-4">
            <div className="bg-white rounded-2xl border border-[#e8e8e8] p-6">
              <h3 className="font-semibold text-black text-[16px] mb-4">Period Snapshot</h3>
              <div className="space-y-3">
                {[
                  ...
                  <div key={r.label} className="flex items-center justify-between py-2 border-b border-[#eeeeee] last:border-none">
                    <span className="text-[13px] text-[#444748]">{r.label}</span>
                    <span className="text-[14px] font-semibold text-black">{r.value}</span>
                  </div>
                ))}
              </div>
            </div>
            ...
            <div className="bg-black rounded-xl p-6 text-white">
              ...
              <p className={`text-[28px] font-['Playfair_Display'] font-semibold ${summary.revenueDelta >= 0 ? "text-[#9f97ff]" : "text-[#ffdad6]"}`}>
        ...
        <section className="bg-white rounded-2xl border border-[#e8e8e8] p-6">
          <h3 className="font-semibold text-black text-[18px] mb-6">Monthly Revenue Breakdown</h3>
              ...
              return (
                <div key={i} className="flex-1 flex flex-col items-center gap-1.5">
                  <div title={`৳${d.curr.toLocaleString()}`}
                    className={`w-full rounded-t-md transition-all ${isHighest ? "bg-[#5951b4]" : "bg-[#eeeeee]"}`}
                    style={{ height: `${Math.max(pct, 3)}%` }} />
                  <span className="text-[11px] text-[#444748]">{d.month}</span>
                </div>
              );
            })}
          </div>
